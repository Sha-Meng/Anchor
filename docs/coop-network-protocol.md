...
13. 双方开始发送 `game.state` 和 `game.event`。

## Demo 场景验收流程

本次 MVP 的工程验收标准是能进入一个基础 demo 场景，完整验证登录、建房、加入房间、开局和自定义协议交互。
...
6. 两个客户端收到 `room.starting`，进入同一个 demo 游戏场景。
7. 两个客户端发送 `room.enteredGame`，服务器广播 `room.inGame`。
8. demo 场景中显示本地玩家和远端玩家。
9. 本地玩家通过 JSON 配置定义的 `game.state` payload 发送基础状态。
10. 远端客户端收到并显示对方状态。
11. 触发一个自定义 `game.event`，例如 `demo.ping` 或 `demo.action`，另一端能收到并在 UI 或日志中显示。
...
- 至少一个自定义 `game.event` 能通过 JSON 配置定义并完成端到端收发。
...
8. 从开发机连接 `ws://43.156.16.10/ws`，确认收到 `system.welcome`。
