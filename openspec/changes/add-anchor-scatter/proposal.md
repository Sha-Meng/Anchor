...
1. 动线点（`RouteNode`）只是攀爬骨架的路径点，本身不是 `AnchorPoint`、没有半径。真正需要半径的是撒出的抓点：策划希望在窗口里直接设定 `AnchorPoint` 的"核心半径"和"最大半径"，并能写入已选中的 `AnchorPoint`，而不是逐个到 Inspector 里改。
...
- 关卡动线编辑器提供 `AnchorPoint` 的"核心半径"（`previewIntenseRadius`）与"最大半径"（`previewSlightRadius`）设置，作为撒点默认值，并可一键写入选中的 `AnchorPoint`。动线点（`RouteNode`）保持不挂 `AnchorPoint`、无半径。
...
- 动线点（`RouteNode`）不挂 `AnchorPoint`；窗口可设置撒点用的核心半径与最大半径，并能写入选中的 `AnchorPoint`。
- 点击"沿动线离散撒点"后，在动线周围生成一批 `AnchorPoint`，且每个锚点都贴在带 `Wall` 脚本的墙面上，两档半径与窗口设置一致。
